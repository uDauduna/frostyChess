...
class Renderer:
    ...
    def _draw_base_board(self):
        ...
        for row in range(8):
            for col in range(8):
                ...
                pygame.draw.rect(self.board_surface, color, (col * 80, row * 80, 80, 80))
    ...
    def draw_board(self, selected=None, legal_moves=None):
        ...
        if self.game.move_history:
            ...
            for square in (move.start, move.end):
                row, col = self.display_square(square)
                pygame.draw.rect(self.screen, self.LAST_MOVE,
                                 (self.board_x + col * 80, self.board_y + row * 80, 80, 80))

        if selected is not None:
            row, col = self.display_square(selected)
            pygame.draw.rect(self.screen, self.SELECTED,
                             (self.board_x + col * 80, self.board_y + row * 80, 80, 80), 4)

        for move in legal_moves or []:
            row, col = self.display_square(move)
            center = (self.board_x + col * 80 + 40, self.board_y + row * 80 + 40)
            target = self.game.board.get_piece(move)
            if target is None:
                pygame.draw.circle(self.screen, self.MOVE_DOT, center, 9)
            else:
                pygame.draw.circle(self.screen, self.CAPTURE_RING, center, 31, 5)
    ...
    def draw(self, pieces_captured_by_black, pieces_captured_by_white, selected_position=None,
             legal_moves=None, black_time=None, white_time=None, dt=0):
        self.draw_board(selected_position, legal_moves)
        self._draw_coordinates()
        self.sync_pieces()

        for piece, sprite in self.sprites.items():
            row, col = self.display_square(piece.position)
            sprite.update_position(self.board_x, self.board_y, self.SQUARE_SIZE, row, col)
            self.screen.blit(sprite.image, sprite.rect)

        self.player_panel.draw(self.screen, pieces_captured_by_black, pieces_captured_by_white,
                               black_time, white_time, self.game.turn, dt)
